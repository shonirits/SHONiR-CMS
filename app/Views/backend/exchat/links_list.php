<?php echo view('backend/'.$cc['backend_theme'].'/common/page_start'); ?>
  ...
  <div class="container">
          ...
          <div class="col-8 p-3">
           ...
           <a href="<?php echo $cc['base_url'].'Links/add'; ?>" class="btn btn-success">Add</a> <button type="button" id="filters" name="filters" class="btn btn-info">Filters</button>
           </div>
           </div>
           <div class="<?php echo ($query)?'show':'hide'?>" id="filters_zone">
           <div class="row">
           <div class="col-6 p-3">
           <form name="filter_frm" id="filter_frm" action="<?php echo $cc['base_url'].'Links/list?query='.$query.'&order='.$order.'&sort='.$sort.'&limit='.$limit; ?>" method="GET" role="form">
           ...
           <thead>
              <tr>
                ...
                <th scope="col" class="align-middle">Public ID<br><small>Name</small><br><small class="text-body-secondary">Today Views-Today Hits-Lifetime View-Lifetime Hits-Votes-Rating-Scores-Likes</small></th>
                <th scope="col" class="text-center align-middle">Type</th>
                <th scope="col" class="text-center align-middle">Host</th>
          ...
          <?php  foreach ($pagination['result'] as $row)
                    { ?>
            <tr>
              <th scope="row" class="text-center align-middle"><?php echo $row['link_id']; ?></th>
              <td class="align-middle"><?php echo $row['public_id'].'<br><small>'.$row['name'].'</small><br><small class="text-body-secondary">'.$row['today_views'].'-'.$row['today_hits'].'-'.$row['lifetime_views'].'-'.$row['lifetime_hits'].'-'.$row['votes'].'-'.$row['ratings'].'-'.$row['scores'].'-'.$row['likes'].'</small>'; ?></td>
               <td class="text-center align-middle"><?php echo $row['parent_type']; ?></td>
               <td class="text-center align-middle"><?php echo host_types_fnc($row['host_id'], 'name'); ?></td>
              ...
              <td class="text-center align-middle"><a href="<?php echo $cc['base_url'].'Links/edit?id='.$row['link_id']; ?>" data-bs-toggle="tooltip" data-bs-original-title="Edit" title="Edit" ><i class="fa-solid fa-pen-to-square"></i></a> <a href="javascript:confirm_fnc('Cancel', '', '', 'Delete', '<?php echo $cc['base_url'].'Links/delete?id='.$row['link_id']; ?>', '', 'Delete Record ID# <?php echo $row['link_id']; ?>', 'Are you sure you want to delete record id# <b><?php echo $row['link_id']; ?></b>, title: <b><?php echo html2text_fnc($row['public_id']); ?></b>?');" data-bs-toggle="tooltip" data-bs-original-title="Delete" title="Delete" ><i class="fa-solid fa-trash-can"></i></a></td>
            </tr>
            <?php } ?>
          </tbody>
          </table>

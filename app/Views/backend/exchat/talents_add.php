<?php echo view('backend/'.$cc['backend_theme'].'/common/page_start'); ?>
  ...
  <form name="add_frm" id="add_frm" action="<?php echo $cc['base_url'].'Talents/add'; ?>" method="POST" role="form" enctype='multipart/form-data' novalidate>
  ...
  <input type="hidden" name="token" id="token" value="<?php echo $form['token']; ?>">
          ...
          <select class="form-control" id="type_id" name="type_id" required>
                    <?php 
          if (!empty($form['type_list'])) {
              foreach ($form['type_list'] as $id => $name) { ?>
                  <option value="<?php echo $id; ?>" <?php echo (!empty($form['type_id']) && in_array($id, (array)$form['type_id'])) ? 'selected="selected"' : ''; ?>>

  ...
  <div class="container">
          ...
          <div class="col-8 p-3">
           ...
           <div class="col-4 p-3 text-end">
            <button type="button" id="filters" name="filters" class="btn btn-info">Filters</button>
           </div>
           </div>
           <div class="<?php echo ($query)?'show':'hide'?>" id="filters_zone">
           <div class="row">
           <div class="col-6 p-3">
           <form name="filter_frm" id="filter_frm" action="<?php echo $cc['base_url'].'Emails/list?query='.$query.'&order='.$order.'&sort='.$sort.'&limit='.$limit; ?>" method="GET" role="form">
           ...
           <thead>
              <tr>
                ...
                <th scope="col" class="align-middle">Email<br><small>Name</small></th>
                <th scope="col" class="text-center align-middle">Status</th>
                <th scope="col" class="text-center align-middle">Subscribed</th>
                <th scope="col" class="text-center align-middle">Verified</th>
                <th scope="col" class="text-center align-middle">Unsubscribed</th>
          ...
          <?php  foreach ($pagination['result'] as $row)
                    { ?>
            <tr>
              <th scope="row" class="text-center align-middle"><?php echo $row['email_id']; ?></th>
              <td class="align-middle"><?php echo $row['email'].'<br><small>'.$row['name'].'</small>'; ?></td>
              ...
              <td class="text-center align-middle"><?php echo ($row['subscribed'] == 1)?'<i class="fa-solid fa-check text-success"></i><br><small>'.date('Y-m-d H:i:s', $row['subscribed_time']).'</small>':'<i class="fa-solid fa-xmark text-danger"></i>'; ?></td>
              <td class="text-center align-middle"><?php echo ($row['verified'] == 1)?'<i class="fa-solid fa-check text-success"></i><br><small>'.date('Y-m-d H:i:s', $row['verified_time']).'</small>':'<i class="fa-solid fa-xmark text-danger"></i>'; ?></td>
              <td class="text-center align-middle"><?php echo ($row['unsubscribed'] == 1)?'<i class="fa-solid fa-check text-success"></i><br><small>'.date('Y-m-d H:i:s', $row['unsubscribed_time']).'</small>':'<i class="fa-solid fa-xmark text-danger"></i>'; ?></td>
              <td class="text-center align-middle"> <a href="javascript:confirm_fnc('Cancel', '', '', 'Delete', '<?php echo $cc['base_url'].'Emails/delete?id='.$row['email_id']; ?>', '', 'Delete Record ID# <?php echo $row['email_id']; ?>', 'Are you sure you want to delete record id# <b><?php echo $row['email_id']; ?></b>, title: <b><?php echo html2text_fnc($row['email']); ?></b>?');" data-bs-toggle="tooltip" data-bs-original-title="Delete" title="Delete" ><i class="fa-solid fa-trash-can"></i></a></td>
            </tr>
            <?php } ?>
          </tbody>
          </table>

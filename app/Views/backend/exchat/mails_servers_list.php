  ...
  <div class="container">
          ...
          <?php  foreach ($pagination['result'] as $row)
            ...
            <tr>
              ...
              <td class="text-center align-middle"><a href="<?php echo $cc['base_url'].'MailsServers/edit?id='.$row['mail_server_id']; ?>" data-bs-toggle="tooltip" data-bs-original-title="Edit" title="Edit" ><i class="fa-solid fa-pen-to-square"></i></a> <a href="javascript:confirm_fnc('Cancel', '', '', 'Delete', '<?php echo $cc['base_url'].'MailsServers/delete?id='.$row['mail_server_id']; ?>', '', 'Delete Record ID# <?php echo $row['mail_server_id']; ?>', 'Are you sure you want to delete record id# <b><?php echo $row['mail_server_id']; ?></b>, title: <b><?php echo html2text_fnc($row['username']); ?></b>?');" data-bs-toggle="tooltip" data-bs-original-title="Delete" title="Delete" ><i class="fa-solid fa-trash-can"></i></a> <?php if(!empty($row['last_error']) && $row['last_error']) { ?><a href="javascript:dialog_fnc('Error', '<?php echo data2js_fnc($row['last_error']); ?>');" data-bs-toggle="tooltip" data-bs-original-title="Error at: <?php echo date("", $row['last_used_time']); ?>" title="Error" ><i class="fa-solid fa-bug text-danger"></i></a><?php } ?></td>
            </tr>
            <?php } ?>
          </tbody>
          </table>

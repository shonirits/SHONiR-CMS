<?php echo view('backend/'.$cc['backend_theme'].'/common/page_start'); ?>
  ...
  <form name="edit_frm" id="edit_frm" action="<?php echo $cc['base_url'].'Categories/edit?id='.$form['id']; ?>" method="POST" role="form" enctype='multipart/form-data' novalidate>
  ...
  <input type="hidden" name="token" id="token" value="<?php echo $form['token']; ?>">
          ...
          <select class="form-control" id="parents" name="parents[]" multiple="multiple" required>
          <option value="0" <?php echo ($form['parents'])?'':'selected="selected" selected'; ?>>---None---</option>
          <?php 
          if($form['db_parents']){
          foreach($form['db_parents'] as $parent){  ?>
          <option value="<?php echo $parent['category_id']?>" <?php echo @(in_array($parent['category_id'], $form['parents']))?'selected="selected" selected':''; ?>><?php echo $parent['name']?></option>
          <?php }}?>
          </select>
          <div class="invalid-feedback">  
          Required valid information. 
          </div>
          <div id="parentshelpblock" class="form-text">
            You can select multiple parents for this records.
          </div>
          </div>

          <div class="row p-3">
          <label for="icon" class="form-label">Icon</label>
          <input type="text" id="icon" name="icon" value="<?php echo $form['icon']; ?>" class="form-control" aria-describedby="iconhelpblock" minlength="1" maxlength="190" required>
          <div class="invalid-feedback">  
          Required valid information. 
          </div>
          <div id="iconhelpblock" class="form-text">
            Add a visual touch by pasting an emoji (😊, 🎬, ⭐) or a symbol (✓, ⌛, ⚡) that represents this item.
          </div>
          </div>

<?php echo view('backend/'.$cc['backend_theme'].'/common/page_start'); ?>
  ...
  <form name="edit_frm" id="edit_frm" action="<?php echo $cc['base_url'].'Pages/edit?id='.$form['id']; ?>" method="POST" role="form" enctype='multipart/form-data' novalidate>
  ...
  <input type="hidden" name="token" id="token" value="<?php echo $form['token']; ?>">
          ...
          <option value="<?php echo $parent['page_id']?>" <?php echo @(in_array($parent['page_id'], $form['parents']))?'selected="selected" selected':''; ?>><?php echo $parent['name']?></option>
          <?php }}?>
          </select>

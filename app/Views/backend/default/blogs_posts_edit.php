<?php echo view('backend/'.$cc['backend_theme'].'/common/page_start'); ?>
  ...
  <form name="edit_frm" id="edit_frm" action="<?php echo $cc['base_url'].'Blogs/posts_edit?id='.$form['id']; ?>" method="POST" role="form" enctype='multipart/form-data' novalidate>
  ...
  <input type="hidden" name="token" id="token" value="<?php echo $form['token']; ?>">
          ...
          <h1>Blogs<h1>
           ...
           <div class="col-8 p-3">
           <h2>Edit Post: <?php echo $form['id']; ?><h2>
           </div>
           <div class="col-4 p-3">
          ...
          <div class="row p-3">
          <label for="blogs_categories" class="form-label">Categories</label>
          <select class="form-control" id="blogs_categories" name="blogs_categories[]" multiple="multiple" required>
          <option value="0" <?php echo ($form['blogs_categories'])?'':'selected="selected" selected'; ?>>---None---</option>
         <?php
$selectedCategoryIds = !empty($form['blogs_categories']) && is_array($form['blogs_categories'])
    ? array_column($form['blogs_categories'], 'blog_category_id')
    : [];

if (!empty($form['db_blogs_categories']) && is_array($form['db_blogs_categories'])) {
    foreach ($form['db_blogs_categories'] as $category) {
        if (!is_array($category) || empty($category['blog_category_id']) || empty($category['name'])) {
            continue; 
        }

        $isSelected = in_array($category['blog_category_id'], $selectedCategoryIds);
        ?>

<?php echo view('backend/'.$cc['backend_theme'].'/common/page_start'); ?>
  ...
  <div class="container">
          ...
          <div class="col-8 p-3">
           ...
           <a href="<?php echo $cc['base_url'].'Blogs/categories_add'; ?>" class="btn btn-success">Add Category</a> <button type="button" id="filters" name="filters" class="btn btn-info">Filters</button>
           </div>
           </div>
           <div class="<?php echo ($query)?'show':'hide'?>" id="filters_zone">
           <div class="row">
           <div class="col-6 p-3">
           <form name="filter_frm" id="filter_frm" action="<?php echo $cc['base_url'].'Blogs/categories_list?query='.$query.'&order='.$order.'&sort='.$sort.'&limit='.$limit; ?>" method="GET" role="form">
          ...
          <?php  foreach ($pagination['result'] as $row)
                    { ?>
            <tr>
              <th scope="row" class="text-center align-middle"><?php echo $row['blog_category_id']; ?></th>
              <td class="align-middle"><?php echo $row['title'].'<br><small>'.$row['name'].'</small><br><small class="text-body-secondary">'.$row['today_views'].'-'.$row['today_hits'].'-'.$row['lifetime_views'].'-'.$row['lifetime_hits'].'-'.$row['votes'].'-'.$row['ratings'].'-'.$row['scores'].'-'.$row['likes'].'</small>'; ?></td>
               <td class="text-center align-middle"><?php echo $row['posts']; ?></td>
              ...
              <td class="text-center align-middle"><a href="<?php echo $cc['base_url'].'Blogs/categories_edit?id='.$row['blog_category_id']; ?>" data-bs-toggle="tooltip" data-bs-original-title="Edit" title="Edit" ><i class="fa-solid fa-pen-to-square"></i></a> <?php if($row['removable']){ ?><a href="javascript:confirm_fnc('Cancel', '', '', 'Delete', '<?php echo $cc['base_url'].'Blogs/categories_delete?id='.$row['blog_category_id']; ?>', '', 'Delete Record ID# <?php echo $row['blog_category_id']; ?>', 'Are you sure you want to delete record id# <b><?php echo $row['blog_category_id']; ?></b>, title: <b><?php echo html2text_fnc($row['title']); ?></b>?');" data-bs-toggle="tooltip" data-bs-original-title="Delete" title="Delete" ><i class="fa-solid fa-trash-can"></i></a><?php } ?></td>
            </tr>
            <?php } ?>
          </tbody>
          </table>
  ...
  var url = base_url+'Blogs/categories_list';
</script>

<?php echo view('backend/'.$cc['backend_theme'].'/common/page_start'); ?>
  ...
  <div class="container">
          ...
          <div class="col-8 p-3">
           ...
           <a href="<?php echo $cc['base_url'].'Blogs/posts_add'; ?>" class="btn btn-success">Add Post</a> <button type="button" id="filters" name="filters" class="btn btn-info">Filters</button>
           </div>
           </div>
           <div class="<?php echo ($query)?'show':'hide'?>" id="filters_zone">
           <div class="row">
           <div class="col-6 p-3">
           <form name="filter_frm" id="filter_frm" action="<?php echo $cc['base_url'].'Blogs/posts_list?query='.$query.'&order='.$order.'&sort='.$sort.'&limit='.$limit; ?>" method="GET" role="form">
          ...
          <?php  foreach ($pagination['result'] as $row)
                    { ?>
            <tr>
              <th scope="row" class="text-center align-middle"><?php echo $row['blog_post_id']; ?></th>
              ...
              <td class="text-center align-middle"><a href="<?php echo $cc['base_url'].'Blogs/posts_edit?id='.$row['blog_post_id']; ?>" data-bs-toggle="tooltip" data-bs-original-title="Edit" title="Edit" ><i class="fa-solid fa-pen-to-square"></i></a> <?php if($row['removable']){ ?><a href="javascript:confirm_fnc('Cancel', '', '', 'Delete', '<?php echo $cc['base_url'].'Blogs/posts_delete?id='.$row['blog_post_id']; ?>', '', 'Delete Record ID# <?php echo $row['blog_post_id']; ?>', 'Are you sure you want to delete record id# <b><?php echo $row['blog_post_id']; ?></b>, title: <b><?php echo html2text_fnc($row['title']); ?></b>?');" data-bs-toggle="tooltip" data-bs-original-title="Delete" title="Delete" ><i class="fa-solid fa-trash-can"></i></a><?php } ?></td>
            </tr>
            <?php } ?>
          </tbody>
          </table>
  ...
  var url = base_url+'Blogs/posts_list';
</script>

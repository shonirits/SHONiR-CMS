...
class Regions extends Shonir_Controller
{
    ...
    public function index()
    {
        ...
        $view_data['page'] = [
            'title' => "Regions | ".$this->cc['app_name'],
           'description' => "",
           'keywords' => "",
           'url' => current_url(),
           'type' => "website"
    ...
    public function list()
    {
        ...
        $order_list = array('region_id' => 'Region ID', 'title' => 'Title', 'name' => 'Name', 'published_time' => 'Published Time', 'today_view' => 'Today View', 'lifetime_view' => 'Lifetime View', 'today_hit' => 'Today Hit', 'lifetime_hit' => 'Lifetime Hit', 'votes' => 'Votes', 'ratings' => 'Ratings', 'add_time' => 'Added Time', 'edit_time' => 'Last Edited Time');
        ...
        if($limit < $default_limit || $page < 1){
            $alert_data['alert'] = ['type' => 'error', 'message' => "The requested record was not found"];
             ...
             return redirect()->to($this->cc['base_url'].'Regions/list');
           }

        $view_data['page'] = [
            'title' => "List | Regions | ".$this->cc['app_name'],
           'description' => "",
           'keywords' => "",
           'url' => current_url(),
           'type' => "website"
         ...
         $total_records = $this->CRUDModel->do_count('regions', $get_result_where, $get_result_like);

        $start = ($page-1)*$limit;

        $get_result = ($total_records > 0)?$this->CRUDModel->get_result('regions', $get_result_where, '', $order.' '.$sort, $limit, $start, $get_result_like):false;

        $url = $this->cc['base_url'].'Regions/list?query='.$query.'&order='.$order.'&sort='.$sort.'&limit='.$limit;
        ...
        return view('backend/'.$this->cc['backend_theme'].'/regions_list', $view_data);
    ...
    public function delete()
    {
        ...
        $get_row_where = array('region_id' => $req_id);                    
        $get_row = $this->CRUDModel->get_row('regions', $get_row_where);
        ...
        $get_row_where_images = array('parent_id' => $req_id, 'parent_type' => 'regions_images');                    
        $get_result_images = $this->CRUDModel->get_result('uploads', $get_row_where_images);

        $get_row_where_gallery = array('parent_id' => $req_id, 'parent_type' => 'regions_gallery');                    
        ...
        $this->CRUDModel->do_delete('regions', $get_row_where);

        $get_delete_children_items = array('region_id' => $req_id); 
        $this->CRUDModel->do_delete('items_to_regions', $get_delete_children_items);
    ...
    public function edit()
    {
        ...
        $view_data['form'] = array('id' => $req_id, 'token' => get_token_fnc(16), 'slug' => $req_slug, 'title' => $req_title, 'name' => $req_name, 'spotlight' => $req_spotlight, 'description' => $req_description, 'status' => $req_status, 'searchable' => $req_searchable, 'listed' => $req_listed, 'featured' => $req_featured, 'top' => $req_top, 'bottom' => $req_bottom, 'today_views' => $req_today_views, 'lifetime_views' => $req_lifetime_views, 'today_hits' => $req_today_hits, 'lifetime_hits' => $req_lifetime_hits, 'votes' => $req_votes, 'ratings' => $req_ratings, 'scores' => $req_scores, 'likes' => $req_likes, 'published_year' => $req_published_year, 'published_month' => $req_published_month, 'published_day' => $req_published_day, 'published_hour' => $req_published_hour, 'published_minute' => $req_published_minute, 'published_second' => $req_published_second, 'meta_title' => $req_meta_title, 'meta_description' => $req_meta_description, 'meta_keywords' => $req_meta_keywords, 'images' => $get_result_images, 'gallery' => $get_result_gallery);
            ...
            if($this->validation->withRequest($this->request)->run() == FALSE){
                ...
                $this->session->set($alert_data);
                    ...
                    $do_count = $this->CRUDModel->do_count('regions', $do_count_where);
                    ...
                    $do_update_where = array('region_id' => $req_id);
                   ...
                   $this->CRUDModel->do_update('regions', $do_update_where, $do_update_data);
                    ...
                    foreach ($req_images as $image)
                        {
                            ...
                            $do_create_data_images =  array('upload_type' => 'image', 'upload_file' => $image_new_name, 'parent_id' => $req_id, 'parent_type' => 'regions_images', 'sort_order' => $var_image_sort_order, 'token' => $req_token, 'add_by' => $user_info['user_id'], 'add_time' => time(), 'add_ip' => $this->request->getIPAddress());
                        ...
                        foreach ($req_gallery as $gallery)
                            {
                                ...
                                $do_create_data_gallery =  array('upload_type' => 'image', 'upload_file' => $gallery_new_name, 'parent_id' => $req_id, 'parent_type' => 'regions_gallery', 'sort_order' => $var_gallery_sort_order, 'token' => $req_token, 'add_by' => $user_info['user_id'], 'add_time' => time(), 'add_ip' => $this->request->getIPAddress());
        ...
        $view_data['page'] = [
            'title' => "Edit | Regions | ".$this->cc['app_name'],
           'description' => "",
           'keywords' => "",
           'url' => current_url(),
           'type' => "website"
       ...
       $view_data['cc'] = $this->cc;
        ...
        return view('backend/'.$this->cc['backend_theme'].'/regions_edit', $view_data);
    ...
    public function add()
    {
        ...
        $view_data['form'] = array('token' => $req_token, 'slug' => $req_slug, 'title' => $req_title, 'name' => $req_name, 'spotlight' => $req_spotlight, 'description' => $req_description, 'status' => $req_status, 'searchable' => $req_searchable, 'listed' => $req_listed, 'featured' => $req_featured, 'top' => $req_top, 'bottom' => $req_bottom, 'published_year' => $req_published_year, 'published_month' => $req_published_month, 'published_day' => $req_published_day, 'published_hour' => $req_published_hour, 'published_minute' => $req_published_minute, 'published_second' => $req_published_second, 'meta_title' => $req_meta_title, 'meta_description' => $req_meta_description, 'meta_keywords' => $req_meta_keywords);
            ...
            if($this->validation->withRequest($this->request)->run() == FALSE){
                ...
                $this->session->set($alert_data);
                   ...
                   $do_create_id = $this->CRUDModel->do_create('regions', $do_create_data);
                    ...
                    foreach ($req_images as $image)
                        {
                            ...
                            $do_create_data_images =  array('upload_type' => 'image', 'upload_file' => $image_new_name, 'parent_id' => $do_create_id, 'parent_type' => 'regions_images', 'sort_order' => $var_image_sort_order, 'token' => $req_token, 'add_by' => $user_info['user_id'], 'add_time' => time(), 'add_ip' => $this->request->getIPAddress());
                        ...
                        foreach ($req_gallery as $gallery)
                            {
                                ...
                                $do_create_data_gallery =  array('upload_type' => 'image', 'upload_file' => $gallery_new_name, 'parent_id' => $do_create_id, 'parent_type' => 'regions_gallery', 'sort_order' => $var_gallery_sort_order, 'token' => $req_token, 'add_by' => $user_info['user_id'], 'add_time' => time(), 'add_ip' => $this->request->getIPAddress());
        ...
        $view_data['page'] = [
            'title' => "Add | Regions | ".$this->cc['app_name'],
           'description' => "",
           'keywords' => "",
           'url' => current_url(),
           'type' => "website"
       ...
       $view_data['cc'] = $this->cc;
        ...
        return view('backend/'.$this->cc['backend_theme'].'/regions_add', $view_data);
